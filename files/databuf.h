...
public:
	...
	explicit OFileDataSource(std::unique_ptr<std::ostream>&& tomove) : OStreamDataSource(nullptr), fout(std::move(tomove)) {
		if (fout) {
			out = fout.get();
		}
	}
...
public:
	...
	explicit OVectorDataSource(std::vector<unsigned char>* existing, bool append = false)
