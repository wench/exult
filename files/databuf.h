...
public:
	...
	explicit IFileDataSource(std::shared_ptr<std::istream>&& shared) : IStreamDataSource(nullptr), pFin(std::move(shared)) {
		if (pFin) {
			in = pFin.get();
			if (in && in->good()) {
				size = get_file_size(*in);
...
public:
	...
	explicit OFileDataSource(File_spec& spec, bool is_text = false) : OStreamDataSource(nullptr) {
		fout = U7open_out(spec.name, is_text);
		out  = fout.get();
	}

	explicit OFileDataSource(std::string_view filename, bool is_text = false) : OStreamDataSource(nullptr) {
		fout = U7open_out(filename, is_text);
		out  = fout.get();
	}

	explicit OFileDataSource(std::shared_ptr<std::ostream>&& shared) : OStreamDataSource(nullptr), fout(std::move(shared)) {
		if (fout) {
			out = fout.get();
		}
	}
...
public:
	OBufferDataSpan(void* data, size_t len) : buf(static_cast<unsigned char*>(data)), buf_ptr(buf), size(len), bad(false) {
		// data can be nullptr if len is also 0
		assert(data != nullptr || len == 0);
	}
...
public:
	explicit OVectorDataSource(size_t initial) : OBufferDataSpan(nullptr, 0), data(&owneddata) {
		ensure_space(initial);
	}

	explicit OVectorDataSource(std::vector<unsigned char, Alloc>* existing, bool append = false)
	...
	explicit OVectorDataSource(std::vector<unsigned char, Alloc>&& existing, bool append = false)
			: OBufferDataSpan(nullptr, 0), owneddata(std::move(existing)), data(&owneddata) {
		if (data) {
			if (append) {
				ensure_space(0);
				skip(data->size());
...
public:
	...
	explicit ODataSource_ostream(ODataSource* ds) : std::ostream(nullptr), dsbuf(ds) {
		init(&dsbuf);
	}

	explicit ODataSource_ostream(std::shared_ptr<ODataSource> ds) : std::ostream(nullptr), dsbuf(ds) {
		init(&dsbuf);
	}
};
#endif
