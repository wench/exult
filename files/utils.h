...
using U7IstreamFactory = std::function<std::shared_ptr<std::istream>(const char* s, std::ios_base::openmode mode)>;
using U7OstreamFactory = std::function<std::shared_ptr<std::ostream>(const char* s, std::ios_base::openmode mode)>;
...
int U7mkdir(std::string_view dirname, int mode = 0755, bool parents = false);
...
std::string_view get_directory_from_path(std::string_view path, bool keepslash = false);
