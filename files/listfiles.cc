...
static int U7ListFilesImp(
		const TA_String<Allocator>& path, TA_FileList<Allocator>& files,
		bool quiet) {
	Allocator<char>  alloc = files.get_allocator();
	...
	if (handle != INVALID_HANDLE_VALUE) {
		...
		do {
			...
			TA_String<Allocator> filename(alloc);
...
static int U7ListFilesImp(
	...
	case 0:    // OK
		for (size_t i = 0; i < globres.gl_pathc; i++) {
			files.push_back(TA_String<Allocator>(globres.gl_pathv[i],alloc));
		}
		globfree(&globres);
		return 0;
	case 3:    // no matches
...
template <template <typename> typename Allocator>
int T_U7ListFiles(
		const TA_String<Allocator>& mask, TA_FileList<Allocator>& files,
		bool quiet) {
	TA_String<Allocator> path(get_system_path(mask));
#ifdef DEBUG
	if (!quiet) {
		std::cerr << "U7ListFiles: " << path << std::endl;
	}
#endif
...
int U7ListFiles(const std::string& mask, FileList& files, bool quiet) {
	return T_U7ListFiles(mask, files, quiet);
}

int U7ListFiles(
		const std::pmr::string&                                  mask,
		ListFiles::TA_FileList<std::pmr::polymorphic_allocator>& files,
		bool                                                     quiet) {
	return T_U7ListFiles(mask, files, quiet);
}
