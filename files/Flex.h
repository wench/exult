...
public:
	Flex_writer(
			ODataSource& o, const char* title, size_t cnt, Flex_header::Flex_vers vers = Flex_header::orig,
...
private:
	Flex_writer(
			Flex_writer* parent, ODataSource& o, const char* title, size_t cnt, Flex_header::Flex_vers vers = Flex_header::orig,
			uint8* buffer = nullptr)
...
public:
	...
	void write_name(const std::string_view& fullname) {
		...
		dout.write(name);
		for (int extra = (8 + 1 + 3 + 1) - int(name.size()); extra > 0; --extra) {
			dout.write1(0);
		}
	}
	...
	Flex_writer start_nested_flex(
			const char* filename, size_t cnt, Flex_header::Flex_vers vers = Flex_header::orig, uint8* buffer = nullptr) {
