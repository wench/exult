...
static U7IstreamFactory istream_factory = [](const char* s, std::ios_base::openmode mode) {
	return std::allocate_shared<std::ifstream>(std::pmr::polymorphic_allocator<std::ifstream>(), s, mode);
};

static U7OstreamFactory ostream_factory = [](const char* s, std::ios_base::openmode mode) {
	return std::allocate_shared<std::ofstream>(std::pmr::polymorphic_allocator<std::ofstream>(), s, mode);
};

// Ugly hack for supporting different paths

static std::map<std::string, std::string, MakeTransparentClass<std::less<std::string_view>>> path_map;
...
std::string_view get_directory_from_path(std::string_view path, bool keepslash) {
	// find last slash or backslash
	auto slash = path.find_last_of("\\/");
	if (slash != std::string_view::npos) {
