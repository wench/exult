...
int Actor_pathfinder_client::get_max_cost(int cost_to_goal    // From estimate_cost().
) const {
...
int Actor_pathfinder_client::check_blocking(const Tile_coord& from, const Tile_coord& to) const {
	if (ignores_npcs()) {
		...
		if (blk) {
			...
			if ((frnum >= Actor::sit_frame && frnum <= Actor::sleep_frame) || blk->get_schedule_type() == Schedule::combat) {
				return -1;
			}
	...
	if (foot.h == 1 && (to.tx == foot.x || to.tx == FIX_COORD(foot.x + foot.w - 1))) {
		return -1;
	} else if (foot.w == 1 && (to.ty == foot.y || to.ty == FIX_COORD(foot.y + foot.h - 1))) {
		return -1;
	}
...
int Actor_pathfinder_client::estimate_cost(const Tile_coord& from, const Tile_coord& to) const {
...
bool Actor_pathfinder_client::at_goal(const Tile_coord& tile, const Tile_coord& goal) const {
...
bool Onecoord_pathfinder_client::at_goal(const Tile_coord& tile, const Tile_coord& goal) const {
	return (goal.tx == -1 || tile.tx == goal.tx) && (goal.ty == -1 || tile.ty == goal.ty) && (goal.tz == -1 || tile.tz == goal.tz);
}
...
Offscreen_pathfinder_client::Offscreen_pathfinder_client(Actor* n, bool ign)
		: Actor_pathfinder_client(n, 0, ign), screen(Game_window::get_instance()->get_win_tile_rect().enlarge(3)),
		  best(-1, -1, -1) {}
...
Offscreen_pathfinder_client::Offscreen_pathfinder_client(
		...
		bool              ign)
		: Actor_pathfinder_client(n, 0, ign), screen(Game_window::get_instance()->get_win_tile_rect().enlarge(3)), best(b) {
...
bool Offscreen_pathfinder_client::at_goal(const Tile_coord& tile, const Tile_coord& goal) const {
...
Approach_object_pathfinder_client::Approach_object_pathfinder_client(Actor* from, const Tile_coord& dest, int dist)
		: Actor_pathfinder_client(from, dist, false), destbox(TileRect(dest.tx, dest.ty, 0, 0).enlarge(dist)) {}
...
Approach_object_pathfinder_client::Approach_object_pathfinder_client(Actor* from, Game_object* to, int dist)
		: Actor_pathfinder_client(from, dist, false), destbox(to->get_footprint().enlarge(dist)) {}

/*
 *  Is tile at goal?
 */

bool Approach_object_pathfinder_client::at_goal(const Tile_coord& tile, const Tile_coord& goal) const {
...
Fast_pathfinder_client::Fast_pathfinder_client(Game_object* from, const Tile_coord& dest, int dist, int mf)
		: Pathfinder_client(mf), destbox(TileRect(dest.tx, dest.ty, 0, 0).enlarge(dist)) {
...
Fast_pathfinder_client::Fast_pathfinder_client(Game_object* from, Game_object* to, int dist, int mf) : Pathfinder_client(mf) {
	init(from, to, dist);
}
...
Fast_pathfinder_client::Fast_pathfinder_client(Actor* from, const Tile_coord& dest, int dist)
		: Pathfinder_client(from->get_type_flags()), destbox(TileRect(dest.tx, dest.ty, 0, 0).enlarge(dist)) {
	init(from, nullptr, dist);
}
...
Fast_pathfinder_client::Fast_pathfinder_client(Actor* from, Game_object* to, int dist) : Pathfinder_client(from->get_type_flags()) {
	init(from, to, dist);
}

void Fast_pathfinder_client::init(Game_object* from, Game_object* to, int dist) {
...
int Fast_pathfinder_client::get_max_cost(int cost_to_goal    // From estimate_cost().
) const {
...
int Fast_pathfinder_client::estimate_cost(const Tile_coord& from, const Tile_coord& to) const {
...
bool Fast_pathfinder_client::at_goal(const Tile_coord& tile, const Tile_coord& goal) const {
	const int dz = tile.tz - goal.tz;    // Got to be on same floor.
	if (dz > 5 || dz < -5) {
		return false;
	}
	const TileRect abox(tile.tx - axtiles + 1, tile.ty - aytiles + 1, axtiles, aytiles);
	return abox.intersects(destbox);
}

static void Get_closest_edge(const Block& fromvol, const Block& tovol, Tile_coord& pos1, Tile_coord& pos2) {
...
bool Fast_pathfinder_client::is_grabable_internal(
	...
	if (path.get_num_steps() == 0) {
		...
		while (path.GetNextStep(t, done)) {
			if (t != from->get_tile() && !client.at_goal(t, dst) && gmap->is_tile_occupied(t)) {
	...
	while (zpath.GetNextStep(t, done)) {
		Game_object* block;
		if (!tovol.has_world_point(t.tx, t.ty, t.tz) && !srcvol.has_world_point(t.tx, t.ty, t.tz)
			&& !fromvol.has_world_point(t.tx, t.ty, t.tz) && gmap->is_tile_occupied(t)
...
bool Fast_pathfinder_client::is_grabable(
	...
	for (int i = 1; i <= MAX_GRAB_DIST; i++) {
		...
		if (is_grabable_internal(from, to->get_center_tile(), to->get_tile(), to->get_block(), client)) {
			return true;
		}
	}
	return false;
...
bool Fast_pathfinder_client::is_grabable(
	...
	for (int i = 1; i <= MAX_GRAB_DIST; i++) {
		Fast_pathfinder_client client(from, to, i, mf);
		if (is_grabable_internal(from, to, to, Block(to.tx, to.ty, to.tz, 1, 1, 1), client)) {
			return true;
		}
	}
	return false;
...
bool Fast_pathfinder_client::is_straight_path(Game_object* from, Game_object* to) {
	...
	bool       done;
	while (path.GetNextStep(t, done)) {
		if (!tovol.has_world_point(t.tx, t.ty, t.tz) && !fromvol.has_world_point(t.tx, t.ty, t.tz) && gmap->is_tile_occupied(t)) {
			return false;    // Blocked.
		}
	}
	return true;    // Looks okay.
...
Monster_pathfinder_client::Monster_pathfinder_client(
		...
		: Fast_pathfinder_client(npc, dest, dist, npc->get_type_flags()), intelligence(npc->get_property(Actor::intelligence)) {}
...
int Monster_pathfinder_client::get_max_cost(int cost_to_goal    // From estimate_cost().
) const {
...
int Monster_pathfinder_client::get_step_cost(
	...
	if (Map_chunk::is_blocked(get_axtiles(), get_aytiles(), get_aztiles(), from, to, get_move_flags())) {
		return -1;
	} else {
		return 1;
	}
