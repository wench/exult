...
bool Configuration::value(const string& key, string& ret, const std::string& defaultvalue) const {
...
bool Configuration::value(const string& key, bool& ret, bool defaultvalue) const {
	const XMLnode* sub = xmltree->subtree(key);
	if (sub) {
