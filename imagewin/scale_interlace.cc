...
void Image_window::show_scaled8to8_interlace(
	...
	const Manip8to8               manip(paletted_surface_palette->colors, inter_surface_format);
	Scale_interlace<unsigned char, uint8, Manip8to8>(
			...
			ibuf->height + guard_band, static_cast<uint8*>(inter_surface->pixels), inter_surface->pitch, manip, scale);
}
...
void Image_window::show_scaled8to16_interlace(
	...
	Scale_interlace<unsigned char, uint16, Manip8to16>(
			...
			inter_surface->pitch / inter_surface_format->bytes_per_pixel, manip, scale);
}
