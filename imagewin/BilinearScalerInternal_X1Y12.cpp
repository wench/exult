...
namespace Pentagram { namespace BilinearScaler {
	...
	BSI_FORCE_INLINE void Interpolate1x6BlockByX1Y12(
			...
			const uint8* const f, uint8*& pixel, const uint_fast32_t pitch, const limit_t limit = nullptr) {
		...
		Interpolate2x2BlockTo1<uintX, Manip, uintS>(a, b, n, n, 256, 256, pixel, limit);
		pixel += pitch;
		Interpolate2x2BlockTo1<uintX, Manip, uintS>(a, b, n, n, 256, (0x500 * 5 / 6) & 0xff, pixel, limit);
		pixel += pitch;
		Interpolate2x2BlockTo1<uintX, Manip, uintS>(b, c, n, n, 256, (0x500 * 4 / 6) & 0xff, pixel, limit);
		pixel += pitch;
		Interpolate2x2BlockTo1<uintX, Manip, uintS>(c, d, n, n, 256, (0x500 * 3 / 6) & 0xff, pixel, limit);
		pixel += pitch;
		Interpolate2x2BlockTo1<uintX, Manip, uintS>(d, e, n, n, 256, (0x500 * 2 / 6) & 0xff, pixel, limit);
		pixel += pitch;
		Interpolate2x2BlockTo1<uintX, Manip, uintS>(e, f, n, n, 256, (0x500 * 1 / 6) & 0xff, pixel, limit);
		pixel += pitch;
	}
	...
	bool BilinearScalerInternal_X1Y12(
		...
		if ((sy + actualblockheight + numyloops * blockheight) < tex_h && !clamp_src && !(sh % blockheight)) {
			numyloops++;
			clip_y    = false;
		...
		while (texel != yloop_end) {
			...
			while (texel != xloop_end) {
				...
				ReadTexelsV<Manip>(actualblockheight, texel, tpitch, a, b, c, d, e, f);
				// advance texel pointer by 1 to the next column
				texel++;

				Interpolate1x6BlockByX1Y12<uintX, Manip, uintS>(a, b, c, d, e, f, pixel, pitch, dst_limit);
		...
		if (clip_y) {
			...
			if (numyloops * blockheight + lines_remaining + sy < tex_h && !clamp_src) {
				// It doesn't matter if this is bigger than blockheight
				lines_remaining = std::max<uint_fast16_t>(255, tex_h - sy - numyloops * blockheight);
			}
