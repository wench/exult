...
namespace Pentagram {
	...
	public:
		...
		inline bool Scale(
				SDL_Surface* texture, sint32 sx, sint32 sy, sint32 sw, sint32 sh, SDL_Surface* dest, sint32 dx, sint32 dy,
				sint32 dw, sint32 dh, bool clamp_src) const {
			const SDL_PixelFormatDetails* src_format  = SDL_GetPixelFormatDetails(texture->format);
			const SDL_PixelFormatDetails* dst_format  = SDL_GetPixelFormatDetails(dest->format);
			SDL_Palette*                  src_palette = SDL_GetSurfacePalette(texture);

			uint8*       pixel = static_cast<uint8*>(dest->pixels) + dx * dst_format->bytes_per_pixel + dy * dest->pitch;
			...
			if (dst_format->bytes_per_pixel == 2) {
				...
				if ((r == 0xf800 && g == 0x7e0 && b == 0x1f) || (b == 0xf800 && g == 0x7e0 && r == 0x1f)) {
					if (src_format->bits_per_pixel == 8 && Scale8To565) {
						const Manip8to565 manip(src_palette->colors, dst_format);
						return Scale8To565(texture, sx, sy, sw, sh, pixel, dw, dh, pitch, clamp_src);
					} else if (src_format->bytes_per_pixel == 2 && Scale565To565) {
					...
					if (src_format->bits_per_pixel == 8 && Scale8To555) {
						const Manip8to555 manip(src_palette->colors, dst_format);
						return Scale8To555(texture, sx, sy, sw, sh, pixel, dw, dh, pitch, clamp_src);
					} else if (src_format->bytes_per_pixel == 2 && Scale555To555) {
