...
namespace Pentagram { namespace BilinearScaler {
	...
	BSI_FORCE_INLINE void WritePix(uint8* dest, uintX val, limit_t limit = nullptr) {
	...
	BSI_FORCE_INLINE uint_fast32_t SimpleLerp8(uint_fast32_t a, uint_fast32_t b, uint_fast32_t fac) {
		return (b << 8) + (a - b) * fac;
	}
	...
	// if fac is 0 the result is b
	BSI_FORCE_INLINE uint_fast32_t SimpleLerp16(uint_fast32_t a, uint_fast32_t b, uint_fast32_t fac) {
		return (b << 16) + (a - (b)) * fac;
	}
	...
	BSI_FORCE_INLINE void Interpolate2x2BlockTo1(
			const uint8* const tl, const uint8* const bl, const uint8* const tr, const uint8* const br, const uint_fast32_t fx,
			const uint_fast32_t fy, uint8* const pixel, limit_t limit = nullptr) {
		if (IsUnclipped(pixel, limit)) {
			WritePix<uintX>(
					pixel,
					Manip::rgb(
							SimpleLerp8(SimpleLerp8(tl[0], tr[0], fx), SimpleLerp8(bl[0], br[0], fx), fy) >> 16,
							SimpleLerp8(SimpleLerp8(tl[1], tr[1], fx), SimpleLerp8(bl[1], br[1], fx), fy) >> 16,
							SimpleLerp8(SimpleLerp8(tl[2], tr[2], fx), SimpleLerp8(bl[2], br[2], fx), fy) >> 16),
					limit);
		}
	}
	...
	BSI_FORCE_INLINE void Interpolate2x2BlockTo2x1(
			const uint8* const tl, const uint8* const bl, const uint8* const tr, const uint8* const br, const uint_fast32_t x1,
			uint_fast32_t x2, uint_fast32_t y, uint8* pixel, const limit_t limit = nullptr) {
		Interpolate2x2BlockTo1<uintX, Manip, uintS>(tl, bl, tr, br, x1, y, pixel, limit);
		Interpolate2x2BlockTo1<uintX, Manip, uintS>(tl, bl, tr, br, x2, y, pixel + sizeof(uintX), limit);
	}
	...
	// is 1 or greater
	BSI_FORCE_INLINE void ReadTexelsV(const uint_fast8_t clipping, texel_t* texel, size_t, Ta& a) {
		// Read a if we can
		if (clipping >= 1) {
	...
	template <class Manip, typename texel_t, typename Ta, typename Tb, typename... Args>
	...
	BSI_FORCE_INLINE void ReadTexelsV(const uint_fast8_t clipping, texel_t* texel, size_t tpitch, Ta& a, Tb& b, Args&... more) {
		// Read a
		...
		// Recursive call to read b and the rest of the args
			...
			ReadTexelsV<Manip>(clipping - 1, texel + tpitch, tpitch, b, more...);
		}
	}
...
#	define InstantiateFunc(func, a, b, c)                                                                                      \
		template bool func<a, b, c>(                                                                                            \
				SDL_Surface*, uint_fast32_t, uint_fast32_t, uint_fast32_t, uint_fast32_t, uint8*, uint_fast32_t, uint_fast32_t, \
				uint_fast32_t, bool);                                                                                           \
