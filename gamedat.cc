...
void GameDat::save_gamedat(
	...
	auto            flex_buffer = gamedat_in_memory.alloc_uc().allocate(
	...
	std::pmr::string pmrname(GSCRNSHOT, gamedat_in_memory.alloc_c());
...
for (const auto* savefile : savefiles) {
		std::pmr::string savefile_pmr(savefile, gamedat_in_memory.alloc_c());
		SaveToFlex(flex, savefile_pmr);
	}
	// Now the Ireg's.
	auto map_buffer = gamedat_in_memory.alloc_uc().allocate(
			Flex_writer::BufferSize(12 * 12));

	std::pmr::vector<uint8_t> outbuf(gamedat_in_memory.alloc_uc());
	outbuf.reserve(
			1024
...
inline std::basic_string<char, std::char_traits<char>, Allocator> GameDat::
		get_save_filename(int num, SaveInfo::Type type, Allocator allocator) {
	// preallocate string to a size that should be big enough
	std::basic_string<char, std::char_traits<char>, Allocator> fname(
			std::size(SAVENAME3) + 4, 0, allocator);
	for (;;) {
...
template std::string GameDat::get_save_filename<std::string::allocator_type>(
		int num, SaveInfo::Type type, std::string::allocator_type);

template std::pmr::string GameDat::get_save_filename<
		std::pmr::string::allocator_type>(
		int num, SaveInfo::Type type, std::pmr::string::allocator_type);
...
void GameDat::save_gamedat(
	...
	auto memlock = gamedat_in_memory.get_lock();
	...
	std::pmr::string fname = get_save_filename(
			index, type, gamedat->gamedat_in_memory.alloc_c());    // Set up name.
...
std::shared_future<void>& GameDat::save_gamedat_async(
		...
		const char*                                 savename) {
	// Lock gamedat in memory during save
	auto gimlock = gamedat_in_memory.get_lock();
	...
	std::pmr::string* filename 
			= p_filename && *p_filename ?gamedat_in_memory.pool.new_object<std::pmr::string>(
                              *p_filename,
							 gamedat_in_memory.alloc_c()):nullptr;
...
void GameDat::read_save_infos() {
	...
	if (save_mask == save_mask && save_infos.size() == filenames.size()) {
		return;
	}
	save_mask = std::move(save_mask);
	save_infos.clear();
...
bool GameDat::save_gamedat_zip(
	...
	zipFile zipfile = zipOpen(
				 std::allocate_shared<OFileDataSource>(
						 gamedat_in_memory.alloc_c(), std::move(out)),
				 gamedat_in_memory.alloc_c());
...
std::future<void> GameDat::writetoMemory(
	...
	auto lock = gamedat_in_memory.get_lock();
	// Starting save so clear 
	gamedat_in_memory.clear();
	// set flag to indicate gamedat in memory is active
	gamedat_in_memory.active = true;
	...
	if (todisk) {
		...
		return std::async(std::launch::async, [this]() {
			...
			if (gamedat_in_memory.active) {
				...
				std::pmr::string fname_str(gamedat_in_memory.alloc_c());
...
GameDat::GamedatInMemory::GamedatInMemory()
		: pool()
		   {
	files = pool.new_object<std::pmr::unordered_map<
			std::pmr::string, std::pmr::vector<unsigned char>>>(alloc_uc());
...
void GameDat::GamedatInMemory::clear() {
	...
	files = pool.new_object<
			std::pmr::unordered_map<
					std::pmr::string, std::pmr::vector<unsigned char>>>(
					alloc_uc());
	save_buffer = static_cast<char*>(pool.allocate(MAX_SAVE_BUFFER));
}
