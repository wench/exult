...
void GameDat::SaveToFlex(
	...
	if (gamedat_in_memory.active) {
		...
		if (it != gamedat_in_memory.files.end()) {
			flex.write_file(std::string_view(fname), it->second.data(), it->second.size());
			return;
		}
	}
	...
	if (!source.good()) {
		...
		throw file_read_exception(fname.c_str());
	}
	size_t size = source.getSize();
	if (size <= MAX_SAVE_BUFFER) {
		source.read(gamedat_in_memory.save_buffer.get(), size);
		flex.write_file(std::string_view(fname), gamedat_in_memory.save_buffer.get(), size);
		return;
	} else {
		// File too big for buffer, let flex read it
		flex.write_file(std::string_view(fname), source);
	}
}
...
void GameDat::save_gamedat(
		const std::string& fname,      // File to create.
...
std::string GameDat::get_save_filename(int num, SaveInfo::Type type) {
	...
	std::string fname(std::size(SAVENAME3) + 4, 0);
	for (;;) {
...
void GameDat::save_gamedat(
	...
	std::string fname = get_save_filename(index, type);    // Set up name.
...
bool GameDat::fileExists(const std::string& fname) {
...
// Level 1 Compression
bool GameDat::Save_level1(zipFile& zipfile, const std::string& fname, bool required) {
	IFileDataSource ds;
...
bool GameDat::Save_level2(zipFile& zipfile, const std::string& fname, bool required) {
	IFileDataSource ds;
	auto*           vec = get_memory_file(fname, false);
...
std::vector<unsigned char>* GameDat::get_memory_file(std::string fname, bool create) {
	...
	if (it != gamedat_in_memory.oldfiles.end()) {
		...
		gamedat_in_memory.oldfiles.erase(it);
		vec->clear();
		return vec;
	}
...
void GameDat::GamedatInMemory::clear() {
	...
	std::swap(files, oldfiles);
	files.clear();

	if (!save_buffer) {
		save_buffer = std::make_unique<char[]>(MAX_SAVE_BUFFER);
	} else {
		std::memset(save_buffer.get(), 0, MAX_SAVE_BUFFER);
	}
}
