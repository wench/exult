...
Newfile_gump::Newfile_gump(bool restore_mode_, bool old_style_mode_)
	...
	if (restore_mode) {
		...
		config->value("config/disk/use_old_style_save_load", old_style_mode, false);
		config->set("config/disk/use_old_style_save_load", old_style_mode ? "yes" : "no", true);
	}
	...
	if (old_style_mode) {
		...
		// Toggle Buttons
		buttons[id_music] = std::make_unique<CallbackToggleButton<Newfile_gump>>(
		...
		buttons[id_speech] = std::make_unique<CallbackToggleButton<Newfile_gump>>(
		...
		buttons[id_effects] = std::make_unique<CallbackToggleButton<Newfile_gump>>(
		...
		HorizontalArrangeWidgets(tcb::span(buttons.data() + id_load, 4), 9, 217);
	}
...
void Newfile_gump::save() {
	...
	if (selected_slot >= SavegameSlots && selected_slot <= LastSlot()) {
		gwin->save_gamedat((*games)[selected_slot - SavegameSlots].num, newname);
...
void Newfile_gump::Slot_widget::paint() {
	...
	for (int i = std::max(0, (newclip.y - sy) / (fieldh + fieldgap)); i < nfg->NumSlots(); i++) {
		...
		} else if (nfg->games && actual_slot >= SavegameSlots && actual_slot <= nfg->LastSlot()) {
...
bool Newfile_gump::mouse_down(
	...
	return scroll->mouse_down(mx, my, button) || Modal_gump::mouse_down(mx, my, button);
...
bool Newfile_gump::mousewheel_up(int mx, int my) {
	return scroll->mousewheel_up(mx, my) || Modal_gump::mousewheel_up(mx, my);
}

bool Newfile_gump::mousewheel_down(int mx, int my) {
	return scroll->mousewheel_down(mx, my) || Modal_gump::mousewheel_down(mx, my);
}
	...
	if (!old_style_mode) {
		...
		for (int i = 0; i < fieldcount; ++i) {
			if (old_games[i].num != i) {
				old_games[i] = SaveInfo(gwin->get_save_filename(i, SaveInfo::REGULAR));
			}
		}
