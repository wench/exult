...
public:
	...
	static inline const String<0x663> Str;

	static inline const String<0x664> Dxt;

	static inline const String<0x665> Int;
...
#	undef DELETE
	...
	static inline auto ordinal_numeral_suffix(int num) {
		if ((num % 10) == 1 && num != 11) {
	...
	static inline const String<0x6D4> GamedatDirectory;

	static inline const String<0x6D5> NewQuickSave;

	static inline const String<0x6D6> EmptySlot;

	static inline const NewlineCString<0x6D0> Unreadable_Savegame;

	static inline const String<0x6D1> NoInfo;

	static inline const String<0x6D7> Settings;

	static inline const String<0x6D8> SaveGames;

	static inline const String<0x6D9> AutosaveCount_;

	static inline const String<0x6DA> QuicksaveCount_;

	static inline const String<0x6DB> SortSavegamesBy_;

	static inline const String<0x6DC> GroupByType_;

	static inline const String<0x6DD> AutosavesWriteToGamedat_;
	...
	static inline const String<0x6E2> Hide_;
};
...
void Newfile_gump::SetTextInputArea(SDL_Window* window) {
	const int line     = selected_slot - list_position();
...
Newfile_gump::Newfile_gump(bool restore_mode_, bool old_style_mode_)
		: Modal_gump(nullptr, gwin->get_width() / 2 - 160, gwin->get_height() / 2 - 100, -1), restore_mode(restore_mode_),
		  old_style_mode(old_style_mode_), tinyfont(fontManager.get_font("TINY_BLACK_FONT")) {
	if (restore_mode) {
	...
	if (old_style_mode) {
		...
		// Toggle Buttons
		widgets[id_music] = std::make_unique<SelfManaged<CallbackToggleButton<Newfile_gump>>>(
				this, &Newfile_gump::toggle_audio_option, old_btn_cols[0], old_btn_rows[1], game->get_shape("gumps/musicbtn"),
				!audio ? 0 : audio->is_music_enabled(), 2, SF_GUMPS_VGA);
		widgets[id_speech] = std::make_unique<SelfManaged<CallbackToggleButton<Newfile_gump>>>(
				this, &Newfile_gump::toggle_audio_option, old_btn_cols[1], old_btn_rows[1], game->get_shape("gumps/speechbtn"),
				!audio ? 0 : audio->is_speech_enabled(), 2, SF_GUMPS_VGA);
		widgets[id_effects] = std::make_unique<SelfManaged<CallbackToggleButton<Newfile_gump>>>(
				this, &Newfile_gump::toggle_audio_option, old_btn_cols[2], old_btn_rows[1], game->get_shape("gumps/soundbtn"),
				!audio ? 0 : audio->are_effects_enabled(), 2, SF_GUMPS_VGA);

		// QUIT
		widgets[id_close] = std::make_unique<SelfManaged<CallbackTextButton<Newfile_gump>>>(
				this, &Newfile_gump::quit, Strings::QUIT(), old_btn_cols[2], old_btn_rows[0], 60);

		// Save
		widgets[id_save] = std::make_unique<SelfManaged<CallbackTextButton<Newfile_gump>>>(
				this, &Newfile_gump::save, Strings::SAVE(), old_btn_cols[1], old_btn_rows[0], 60);
		// Load
		widgets[id_load] = std::make_unique<SelfManaged<CallbackTextButton<Newfile_gump>>>(
				this, &Newfile_gump::load, Strings::LOAD(), old_btn_cols[0], old_btn_rows[0], 60);

		HorizontalArrangeWidgets(tcb::span(widgets.data() + id_normal_start, id_settings_start - id_normal_start), 9);
		HorizontalArrangeWidgets(tcb::span(widgets.data() + id_normal_start, id_settings_start - id_normal_start), 9);
		...
		// Cancel
		widgets[id_close] = std::make_unique<SelfManaged<CallbackTextButton<Newfile_gump>>>(
				this, &Newfile_gump::close, Strings::CANCEL(), btn_cols[3], btn_rows[0]);
		// Save
		widgets[id_save] = std::make_unique<SelfManaged<CallbackTextButton<Newfile_gump>>>(
				this, &Newfile_gump::save, Strings::SAVE(), btn_cols[0], btn_rows[0]);
		// Load
		widgets[id_load] = std::make_unique<SelfManaged<CallbackTextButton<Newfile_gump>>>(
				this, &Newfile_gump::load, Strings::LOAD(), btn_cols[1], btn_rows[0]);
		// Delete
		widgets[id_delete] = std::make_unique<SelfManaged<CallbackTextButton<Newfile_gump>>>(
				this, &Newfile_gump::delete_file, Strings::DELETE(), btn_cols[2], btn_rows[0]);

		widgets[id_change_mode] = std::make_unique<SelfManaged<CallbackToggleTextButton<Newfile_gump>>>(
				this, &Newfile_gump::toggle_settings, std::vector<std::string>{Strings::Settings(), Strings::SaveGames()}, 0,
				btn_cols[3], btn_rows[0], 0, 0);

		HorizontalArrangeWidgets(tcb::span(widgets.data() + id_normal_start, id_settings_start - id_normal_start), 9, 217);

		// Apply button uses same location as save button
		widgets[id_apply] = std::make_unique<SelfManaged<CallbackTextButton<Newfile_gump>>>(
				this, &Newfile_gump::apply_settings, Strings::APPLY(), 1, btn_rows[0]);

		// Revert button uses same location as load button
		widgets[id_revert] = std::make_unique<SelfManaged<CallbackTextButton<Newfile_gump>>>(
				this, &Newfile_gump::revert_settings, Strings::REVERT(), 2, btn_rows[0]);

		HorizontalArrangeWidgets(tcb::span(widgets.data() + id_apply, 2), 9, widgets[id_close]->get_x());
		...
		int num_width = std::max(
				...
				std::max(
						font->get_text_width(std::to_string(fmaxval).c_str()),
		...
		widgets[id_slider_flagautocount] = std::make_unique<Slider_widget>(
				this, get_button_pos_for_label(Strings::GlobalFlagAutosaveCount_()), yForRow(yindex++) - 12, std::nullopt,
				std::nullopt, std::nullopt, Settings::get().disk.flagautosave_count.get_min(), fmaxval, 1,
				Settings::get().disk.flagautosave_count, 64, font, num_width, true, false);
		...
		widgets[id_button_groupbytype] = std::make_unique<SelfManaged<Gump_ToggleTextButton>>(
				...
				get_button_pos_for_label(Strings::GroupByType_()), yForRow(yindex++) - 2, 64, 0);

		widgets[id_button_autosaves_write_to_gamedat] = std::make_unique<SelfManaged<Gump_ToggleTextButton>>(
				this, std::vector<std::string>{Strings::No(), Strings::Yes()}, Settings::get().disk.autosaves_write_to_gamedat,
...
void Newfile_gump::save() {
	// Shouldn't ever happen.
	if ((!strlen(newname) && selected_slot != QuicksaveSlot) || selected_slot < FirstSlot() || restore_mode) {
		return;
	}
...
void Newfile_gump::toggle_settings(int state) {
	...
	if (!page_turn_effect) {
		page_turn_effect = std::make_unique<PageTurnEffect>(transition_duration, usable, 0, false);
	} else {
...
bool Newfile_gump::run() {
	bool need_repaint = Modal_gump::run() || (page_turn_effect && page_turn_effect->isStarted());
...
void Newfile_gump::Slot_widget::paint() {
	...
	for (int i = std::max(0, (newclip.y - sy) / (fieldh + fieldgap)); i < nfg->NumSlots(); i++) {
		...
		ibuf->draw_beveled_box(fx, fy, fieldw, fieldh, 1, 137, 144, 144, 140, 140, 142);
		...
		if (nfg->selected_slot == actual_slot && nfg->cursor != -1) {
			gwin->get_win()->fill8(
					0, 1, sman->get_text_height(2), fx + textx + sman->get_text_width(2, text, nfg->cursor), fy + texty);
		}
	}
}
...
void Newfile_gump::paint() {
	if (page_turn_effect && page_turn_effect->isStarted() && !page_turn_effect->isComplete()) {
...
void Newfile_gump::paint_normal() {
	...
	if (details && party && party->size()) {
		for (int i = party->size() - 1; i >= 0; --i) {
			...
			if (i == 0 && !sman->have_si_shapes() && Shapeinfo_lookup::IsSkinImported(shape_num) && shape_file == SF_SHAPES_VGA) {
		...
		if (filename) {
			...
			if (cursize < std::size(info) - 2) {
				...
				cursize += svfile.copy(info + cursize, std::min(std::size(info) - cursize, svfile.size()));

				info[cursize++] = ':';

				if (cursize < std::size(info)) {
					cursize += svfilename.copy(info + cursize, std::min(std::size(info) - cursize, svfilename.size()));
				}
		...
		if (filename) {
			...
			tinyfont->paint_text_box(ibuf, info, x + infox, y + infoy, infow, infoh, 0, false, false);
		}
...
void Newfile_gump::paint_settings() {
	...
	for (int i = id_first; i < id_count; i++) {
		...
		if (widget) {
			...
			//}
			// Zebra striping
			if (i >= id_slider_autocount && i <= id_button_autosaves_write_to_gamedat) {
				ibuf->draw_box(sx + usable.x, rect.y + 2, usable.w, rect.h - 4, 0, 143, 142);
			}
			widget->paint();
		}
	}

	font->paint_text(ibuf, Strings::AutosaveCount_(), sx + label_margin, sy + yForRow(y_index));
	font->paint_text(ibuf, Strings::GlobalFlagAutosaveCount_(), sx + label_margin, sy + yForRow(++y_index));
	font->paint_text(ibuf, Strings::QuicksaveCount_(), sx + label_margin, sy + yForRow(++y_index));
	font->paint_text(ibuf, Strings::SortSavegamesBy_(), sx + label_margin, sy + yForRow(++y_index));
	font->paint_text(ibuf, Strings::GroupByType_(), sx + label_margin, sy + yForRow(++y_index));
	font->paint_text(ibuf, Strings::AutosavesWriteToGamedat_(), sx + label_margin, sy + yForRow(++y_index));
}
...
inline bool Newfile_gump::forward_input(std::function<bool(Gump_widget*)> func) {
	...
	bool doing_transition = page_turn_effect && page_turn_effect->isStarted() && !page_turn_effect->isComplete();
...
bool Newfile_gump::mouse_up(
	...
	bool result = Modal_gump::mouse_up(mx, my, button) || forward_input([mx, my, button](Gump_widget* widget) {
...
bool Newfile_gump::mousewheel_up(int mx, int my) {
	return Modal_gump::mousewheel_up(mx, my) || forward_input([mx, my](Gump_widget* widget) {
			   return widget->mousewheel_up(mx, my);
		   });
}

bool Newfile_gump::mousewheel_down(int mx, int my) {
	return Modal_gump::mousewheel_down(mx, my) || forward_input([mx, my](Gump_widget* widget) {
...
bool Newfile_gump::mouse_drag(
	...
	return Modal_gump::mouse_drag(mx, my) || forward_input([mx, my](Gump_widget* widget) {
...
int Newfile_gump::AddCharacter(char c) {
	...
	text[cursor] = c;
	strncpy(text + cursor + 1, newname + cursor, GameDat::MAX_SAVEGAME_NAME_SIZE - cursor - 1);
