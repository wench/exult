	...
	if (selected_slot == EmptySlot) {
		...
		if (screenshot && (*games)[savegame_index].palette) {
			...
			// one and palette 0 palette to brighten it up slightly
					...
					(*games)[savegame_index].palette->get_palette_index() != gwin->get_pal()->get_palette_index()
					&& (*games)[savegame_index].palette->get_palette_index() != 0) {
				Palette palette0;
				palette0.load(PALETTES_FLX, PATCH_PALETTES, 0);

				newpal = (*games)[savegame_index].palette->create_intermediate(palette0, 3, 1, false);
			}
			palette_map = sg_palette_map;
			if (newpal) {
				newpal->create_palette_map(gwin->get_pal(), sg_palette_map, true);
			} else if ((*games)[savegame_index].palette->get_palette_index() != gwin->get_pal()->get_palette_index()) {
				(*games)[savegame_index].palette->create_palette_map(gwin->get_pal(), sg_palette_map, true);
			} else {
				palette_map = nullptr;
			}
		}
