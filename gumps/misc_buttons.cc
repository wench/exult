...
Arrow_Button::Arrow_Button(Gump_Base* parent, int px, int py, Direction direction, bool double_arrow)
		: Basic_button(parent, px, py, direction >= Left ? 12 : 11, direction >= Left ? 11 : 12), direction(direction),
		  double_(double_arrow) {}
	...
	if (!double_) {
		...
		for (int l = 0; l < 4; l++) {
			...
			if (startx == endx && starty == endy) {
				...
				ib8->draw_line8(0, draw_area.x + startx, draw_area.y + starty, draw_area.x + endx, draw_area.y + endy);
			}
		}
