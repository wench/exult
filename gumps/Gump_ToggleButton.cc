...
Gump_ToggleButton::Gump_ToggleButton(Gump* par, int px, int py, int shapenum, int selectionnum, int numsel, ShapeFile shfile)
		: Toggle_button(numsel, par, shapenum, px, py, shfile) {
	set_frame(selectionnum);
}

Gump_ToggleTextButton::Gump_ToggleTextButton(
		Gump_Base* par, const std::vector<std::string>& s, int selectionnum, int px, int py, int width, int height)
...
Gump_ToggleTextButton::Gump_ToggleTextButton(
		Gump_Base* par, std::vector<std::string>&& s, int selectionnum, int px, int py, int width, int height)
		: Toggle_button(s.size(), par, "", px, py, width, height), selections(std::move(s)) {
	set_frame(selectionnum);
	// call init for all of the strings to ensure the widget is wide enough
	// for all of them
	for (auto& selection : selections) {
