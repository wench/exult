...
Slider_widget::Slider_widget(
		...
		std::optional<ShapeID> osidDiamond, int mival, int mxval, int step, int defval, int width, bool logarithmic)
	...
	if (osidLeft) {
		...
		left     = std::make_unique<SelfManaged<CallbackButton<Slider_widget>>>(
                this, &Slider_widget::clicked_left_arrow, sid.get_shapenum(), 0, 0, sid.get_shapefile());
	} else {
		left = std::make_unique<SelfManaged<CallbackButtonBase<Slider_widget, Arrow_Button>>>(
				this, &Slider_widget::clicked_left_arrow, 0, 0, Arrow_Button::Left, false);
	}
	if (osidRight) {
		auto sid = osidRight.value_or(ShapeID(-1, -1, SF_OTHER));
		right    = std::make_unique<SelfManaged<CallbackButton<Slider_widget>>>(
                this, &Slider_widget::clicked_right_arrow, sid.get_shapenum(), 0, 0, sid.get_shapefile());
	} else {
		right = std::make_unique<SelfManaged<CallbackButtonBase<Slider_widget, Arrow_Button>>>(
				this, &Slider_widget::clicked_right_arrow, 0, 0, Arrow_Button::Right, false);
	}
	...
	diamond->set_pos(0, buttonbottom - diamond->get_ybelow() - (left->get_height() - diamond->get_height()) / 2);
...
void Slider_widget::Diamond::paint() {
	...
	for (const auto& line : lines) {
		if (line.ispoint()) {
			...
			ibuf8->draw_line8(line.col, x + line.startx, y + line.starty, x + line.endx, y + line.endy);
		}
	}
}
...
bool Slider_widget::mouse_down(
	...
	if (diamond->on_widget(mx, my)) {
		...
		if (ly < diamond->get_y() || ly > diamond->get_y() + diamond->get_height()) {
			return Gump_widget::mouse_down(mx, my, button);
		} else if (lx < xmin - diamond->get_width() || lx >= xmax + diamond->get_width()) {
			return Gump_widget::mouse_down(mx, my, button);
		} else if (lx < xmin) {
			lx = xmin;
		} else if (lx > xmax) {
