...
protected:
	...
	int                         last_selected = InvalidSlot;    // keeping track of the selected line for iOS
...
private:
	...
	public:
		Slot_widget(Newfile_gump* nfg) : Gump_widget(nfg, 0, 0, 0, -1, SF_OTHER), nfg(nfg) {}

		bool mouse_down(int mx, int my, MouseButton button) override;

		TileRect get_rect() const override {
			TileRect ret(0, 0, fieldw, (fieldh + fieldgap) * nfg->NumSlots() - fieldgap);
