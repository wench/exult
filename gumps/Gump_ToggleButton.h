...
protected:
	...
	Toggle_button(int numselections, Ts&&... args) : base(std::forward<Ts>(args)...), numselections(numselections) {}
...
public:
	Gump_ToggleButton(Gump* par, int px, int py, int shapenum, int selectionnum, int numsel, ShapeFile shfile = SF_EXULT_FLX);
...
public:
	...
	Gump_ToggleTextButton(
			Gump_Base* par, std::vector<std::string>&& s, int selectionnum, int px, int py, int width, int height = 0);
...
public:
	...
	CallbackToggleButton(Parent* par, CallbackType&& callback, Ts&&... args)
			: Gump_ToggleButton(par, std::forward<Ts>(args)...), parent(par), on_toggle(std::forward<CallbackType>(callback)) {}

	template <typename... Ts>
	CallbackToggleButton(Parent* par, CallbackType2&& callback, Ts&&... args)
			: Gump_ToggleButton(par, std::forward<Ts>(args)...), parent(par), on_toggle2(std::forward<CallbackType2>(callback)) {}
