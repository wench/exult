...
public:
	...
	struct GamedatInMemory {
		...
		std::pmr::polymorphic_allocator<char> alloc_c() {
			return &pool;
		}

		std::pmr::polymorphic_allocator<unsigned char> alloc_uc() {
			return &pool;
		}
		...
		auto get_lock() {
			// weird syntax to suppress msvc code analysis warning C26110
			auto& l = mutexRef();
			return std::unique_lock<std::recursive_mutex>(l);
		}
...
public:
	...
	std::basic_string<char, std::char_traits<char>, Allocator> get_save_filename(
			int num, SaveInfo::Type type, Allocator allocator);
	...
	auto Open_ODataSource(const char* fname) {
		return ODataSourceFileOrVector<std::pmr::polymorphic_allocator>(
				get_memory_file(fname, true),
				std::pmr::string(fname, gamedat_in_memory.alloc_c()));
	}
	...
	std::shared_ptr<std::ostream> Open_ostream(const char* fname) {
		...
		std::shared_ptr<ODataSource> ds
				= std::allocate_shared<OVectorDataSource<
						...
						gamedat_in_memory.alloc_uc(), memfile);

		return std::allocate_shared<ODataSource_ostream>(
				gamedat_in_memory.alloc_c(), ds);
	}
...
private:
	...
	std::pmr::vector<unsigned char>* get_memory_file(
			const char* fname, bool create) {
		return get_memory_file(
				std::pmr::string(fname, gamedat_in_memory.alloc_c()), create);
	}
	...
	std::shared_ptr<IDataSource> Open_IDataSource(std::pmr::string& fname) {
		...
		if (memfile) {
			return std::allocate_shared<IBufferDataView>(
					gamedat_in_memory.alloc_c(), memfile->data(),
					memfile->size());
		} else {
			return std::allocate_shared<IFileDataSource>(
					gamedat_in_memory.alloc_c(),
					std::move(U7open_in(fname, false)));
		}
	}
	...
	bool fileExists(const char* fname) {
		return fileExists(std::pmr::string(fname, gamedat_in_memory.alloc_c()));
	}
	...
	bool Save_level2(
		...
		return Save_level2(
				zipfile, std::pmr::string(fname, gamedat_in_memory.alloc_c()),
				required);
	}
...
public:
	...
	auto getTempAlloc() {
		return gamedat_in_memory.alloc_c();
	}
