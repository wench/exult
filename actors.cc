...
void Main_actor::switched_chunks(
	...
	if (!olist ||     // No old, or new map?  Use all 9.
		...
		int oldsc  = (oldcy / c_chunks_per_schunk) * c_num_schunks + (oldcx / c_chunks_per_schunk);
		int newsc  = (newcy / c_chunks_per_schunk) * c_num_schunks + (newcx / c_chunks_per_schunk);
