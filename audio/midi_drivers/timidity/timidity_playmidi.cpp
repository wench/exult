...
#	endif
	...
	sint32 control_ratio = 0, amplification = DEFAULT_AMPLIFICATION;
	...
	static void select_sample(int v, Instrument* ip) {
		...
		/*
		 ...
		 frequency is closest to the one we want. (Actually we should
												   probably convert the low,
		 high, and root frequencies to MIDI note values and compare those.) */
...
#	define MIDIEVENT(t, ch, pa, pb)            \
		...
		event.type    = t;                      \
		event.channel = ch;                     \
		event.a       = static_cast<uint8>(pa); \
		event.b       = static_cast<uint8>(pb);
