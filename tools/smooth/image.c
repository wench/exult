...
int img_read(const char* filein) {
	...
	g_statics.image_in_format  = SDL_GetPixelFormatDetails(g_statics.image_in->format);
	...
	if (g_statics.image_in_format->bits_per_pixel < 8) {
		...
		SDL_Surface* converted8 = SDL_ConvertSurface(g_statics.image_in, SDL_PIXELFORMAT_INDEX8);
		if (converted8 == NULL) {
		...
		if (converted8_palette == NULL || converted8_palette->ncolors < g_statics.image_in_ncolors) {
			fprintf(stderr, "ERROR: Palette build : %s\n", SDL_GetError());
			SDL_DestroySurface(converted8);
			SDL_DestroySurface(g_statics.image_in);
			return -1;
		}
		if (!SDL_SetPaletteColors(converted8_palette, g_statics.image_in_palette->colors, 0, g_statics.image_in_ncolors)) {
	...
	if ((g_variables.image_out = SDL_DuplicateSurface(g_statics.image_in)) == NULL) {
		fprintf(stderr, "ERROR: %s\n", SDL_GetError());
		SDL_DestroySurface(g_statics.image_in);
		SDL_CloseIO(rw);
		return -1;
	}
	g_variables.image_out_format = SDL_GetPixelFormatDetails(g_variables.image_out->format);
	if ((g_variables.image_out_palette = SDL_CreateSurfacePalette(g_variables.image_out)) == NULL
		|| g_variables.image_out_palette->ncolors < g_statics.image_in_ncolors) {
		fprintf(stderr, "ERROR: Palette build : %s\n", SDL_GetError());
	...
	if (!SDL_SetPaletteColors(g_variables.image_out_palette, g_statics.image_in_palette->colors, 0, g_statics.image_in_ncolors)) {
		fprintf(stderr, "ERROR: Palette transfer : %s\n", SDL_GetError());
		SDL_DestroySurface(g_variables.image_out);
		SDL_DestroySurface(g_statics.image_in);
		SDL_CloseIO(rw);
...
int img_write(const char* img_out) {
	...
	if (g_variables.image_out_palette->ncolors > g_variables.image_out_ncolors) {
...
Uint8 getpixel(SDL_Surface* surface, const SDL_PixelFormatDetails* surface_format, int x, int y) {
...
colour_hex transform(int index) {
	...
	colour_hex ret
			= ((((colour_hex)(colors[index].r)) << 16) | (((colour_hex)(colors[index].g)) << 8)
...
Uint8 palette_rw(colour_hex col) {
	...
	int idx = SDL_MapRGB(g_variables.image_out_format, g_variables.image_out_palette, r, g, b);
...
int process_image(void) {
	...
	for (g_variables.global_y = 0; g_variables.global_y < 192; g_variables.global_y++) {
		for (g_variables.global_x = 0; g_variables.global_x < 192; g_variables.global_x++) {
			Uint8 idx = getpixel(g_statics.image_in, g_statics.image_in_format, g_variables.global_x, g_variables.global_y);
