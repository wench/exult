...
static GimpProcedure* shp_create_procedure(GimpPlugIn* plug_in, const gchar* name);

static GimpValueArray* shp_load(
		GimpProcedure* procedure, GimpRunMode run_mode, GFile* file, GimpMetadata* metadata, GimpMetadataLoadFlags* flags,
		GimpProcedureConfig* config, gpointer run_data);
static GimpValueArray* shp_export(
		GimpProcedure* procedure, GimpRunMode run_mode, GimpImage* image, GFile* file, GimpExportOptions* options,
		GimpMetadata* metadata, GimpProcedureConfig* config, gpointer run_data);
static gboolean shp_palette_dialog(const gchar* title, GimpProcedure* procedure, GimpProcedureConfig* config);

static gint       load_palette(GFile* file, guchar palette[], GError** error);
static GimpImage* load_image(GFile* file, GFile* palette_file, GimpRunMode run_mode, GError** error);
static gboolean   export_image(GFile* file, GimpImage* image, GimpRunMode run_mode, GError** error);
...
static GimpProcedure* shp_create_procedure(GimpPlugIn* plug_in, const gchar* name) {
	...
	if (!strcmp(name, LOAD_PROC)) {
		procedure = gimp_load_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, shp_load, nullptr, nullptr);
		...
		gimp_procedure_set_documentation(
				...
				"Shape files.",
				name);
		gimp_procedure_set_attribution(procedure, "The Exult Team", "The Exult Team", "2010-2025");
		gimp_file_procedure_set_extensions(GIMP_FILE_PROCEDURE(procedure), "shp");
		gimp_procedure_add_file_argument(
				procedure, "palette-file", "_Palette file", "PAL file to load palette from", GIMP_FILE_CHOOSER_ACTION_OPEN, TRUE,
				nullptr, G_PARAM_READWRITE);
	} else if (!strcmp(name, EXPORT_PROC)) {
		procedure = gimp_export_procedure_new(plug_in, name, GIMP_PDB_PROC_TYPE_PLUGIN, FALSE, shp_export, nullptr, nullptr);
		...
		gimp_export_procedure_set_capabilities(
				...
				static_cast<GimpExportCapabilities>(
						GIMP_EXPORT_CAN_HANDLE_RGB | GIMP_EXPORT_CAN_HANDLE_ALPHA | GIMP_EXPORT_CAN_HANDLE_LAYERS
						| GIMP_EXPORT_CAN_HANDLE_INDEXED),
				nullptr, nullptr, nullptr);
		gimp_procedure_add_file_argument(
				procedure, "palette-file", "_Palette file", "PAL file to save palette to", GIMP_FILE_CHOOSER_ACTION_OPEN, TRUE,
				nullptr, G_PARAM_READWRITE);
	}
	return procedure;
}
...
static GimpValueArray* shp_load(
	...
	return_vals = gimp_procedure_new_return_values(procedure, GIMP_PDB_SUCCESS, nullptr);
...
static gboolean shp_palette_dialog(const gchar* title, GimpProcedure* procedure, GimpProcedureConfig* config) {
	...
	dialog = gimp_procedure_dialog_new(GIMP_PROCEDURE(procedure), GIMP_PROCEDURE_CONFIG(config), title);
...
static guchar gimp_cmap[768]
		= {};
...
static gint load_gpl_palette(const gchar* path) {
	...
	if (!std::getline(infile, line) || line.find("GIMP Palette") == std::string::npos) {
		return 0;
	}
...
static GimpImage* load_image(GFile* file, GFile* palette_file, GimpRunMode run_mode, GError** error) {
	...
	GimpImage* image = gimp_image_new(bounds.xleft + bounds.xright + 1, bounds.yabove + bounds.ybelow + 1, GIMP_INDEXED);
	...
	for (auto& frame : shape) {
		...
		GimpLayer*        layer     = gimp_layer_new(
                image, framename.c_str(), frame->get_width(), frame->get_height(), image_type, 100,
                gimp_image_get_default_new_layer_mode(image));
		gimp_image_insert_layer(image, layer, nullptr, 0);
		gimp_item_transform_translate(GIMP_ITEM(layer), bounds.xleft - frame->get_xleft(), bounds.yabove - frame->get_yabove());

		GeglBuffer*         drawable = gimp_drawable_get_buffer(GIMP_DRAWABLE(layer));
		const GeglRectangle rect{0, 0, gegl_buffer_get_width(drawable), gegl_buffer_get_height(drawable)};

		Image_buffer8       img(frame->get_width(),
		...
		gegl_buffer_set(drawable, &rect, 0, nullptr, pixel_data, GEGL_AUTO_ROWSTRIDE);
...
static gboolean export_image(GFile* file, GimpImage* image, GimpRunMode run_mode, GError** error) {
	...
	for (auto& frame : shape) {
		...
		const GeglRectangle rect{0, 0, gegl_buffer_get_width(drawable), gegl_buffer_get_height(drawable)};
		gegl_buffer_get(drawable, &rect, 1.0, format, pix.data(), GEGL_AUTO_ROWSTRIDE, GEGL_ABYSS_NONE);
		g_object_unref(drawable);
		...
		frame     = std::make_unique<Shape_frame>(outptr, width, height, xleft, yabove, has_alpha);
