...
namespace {
	...
	bool saveFrameToPNG(
			const std::string& filename, const unsigned char* data, size_t width, size_t height, const Palette_t& palette) {
	...
	// Import SHP to PNG
	bool importSHP(const std::string& shpFilename, const std::string& outputPngFilename, const std::string& paletteFile) {
		...
		for (int i = 0; i < frameCount; i++) {
			...
			std::string sanitizedFrameFilename    = sanitizeFilePath(frameFilename);
			std::string sanitizedMetadataFilename = sanitizeFilePath(metadataFilename);
...
// Write frame info to metadata file
			...
			// Unix/Mac implementation with restricted permissions (0644 =
			// rw-r--r--)
			int   fd       = open(sanitizedMetadataFilename.c_str(), O_WRONLY | O_CREAT | O_TRUNC, 0644);
			...
			if (!saveFrameToPNG(sanitizedFrameFilename, pixels, frameWidth, frameHeight, palette)) {
	...
	// Export PNG to SHP
	bool exportSHP(
			const std::string& basePath, const std::string& outputShpFilename, int defaultOffsetX, int defaultOffsetY,
		...
		while (fgets(line, sizeof(line), metaFile) != nullptr) {
			...
			if (strncmp(line, "num_frames=", 11) == 0) {
				...
				offsets.resize(numFrames, std::make_pair(defaultOffsetX, defaultOffsetY));
			} else if (strncmp(line, "frame", 5) == 0 && (strstr(line, "offset_x=") != nullptr)) {
			...
			} else if (strncmp(line, "frame", 5) == 0 && (strstr(line, "offset_y=") != nullptr)) {
		...
		while (frameIdx < numFrames) {
			...
			if (color_type != PNG_COLOR_TYPE_PALETTE) {
				std::cerr << "Error: Frame " << frameIdx << " is not an indexed color image" << std::endl;
				png_destroy_read_struct(&png_ptr, &info_ptr, nullptr);
				fclose(fp);
				return false;
			}
			...
			unsigned char transparentIndex = 255;    // Default transparent index in shapes
			...
			std::cout << "Frame " << frameIdx << ": " << width << "x" << height << " offset(" << xleft << "," << yabove << ")"
					  << std::endl;
...
int main(int argc, char* argv[]) {
	if (argc < 3) {
		...
		std::cout << "  Import mode: " << argv[0] << " import <shp_file> <output_png> [palette_file]" << std::endl;
	...
	if (mode == "import") {
		...
		std::cout << "Palette file: " << (!paletteFile.empty() ? paletteFile : "default") << std::endl;
	...
	if (mode == "export") {
		...
		if (exportSHP(sanitizedPngPath, sanitizedShpPath, offsetX, offsetY, metadataPath)) {
