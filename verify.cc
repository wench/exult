...
struct FileHash {
	...
	FileHash(const char* hash, const char* alternative = nullptr) : hash(hash), alternative(alternative) {}
};
...
int verify_files(BaseGameInfo* game) {
	...
	const auto         get_filelist = [&empty_map, game]() -> const File2HashMap& {
        ...
        case BLACK_GATE:
            ...
            switch (game ? game->get_game_language() : Game::get_game_language()) {
	...
	for (const auto& file_hash : files) {
		...
		if (!hashgood) {
			errors++;
			std::cout << "Hash mismatch for file '" << file << "':" << std::endl << "\tExpected hash: " << hash.hash << std::endl;
			if (hash.alternative) {
	...
	std::cout << "========================================" << std::endl << "File verification finished" << std::endl;
	if (errors == 0) {
		...
		std::cout << errors << ' ' << file_or_files << " did not match the expected " << hash_or_hashes << "; see the affected "
				  << file_or_files << " above." << std::endl
				  << "If you did not modify " << these_or_this << ' ' << file_or_files << ", " << they_are_or_it_is
...
const File2HashMap blackgate_files{
		...
		{     USECODE, {"570aa419ee8ccd76343162b59ec71fcbea72e1a7", "272653ec0075da961f9393c716da0baf5f3e19f1"}},
