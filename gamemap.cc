...
void Game_map::write_gamedat_objs() {
	...
	for (int schunk = 0; schunk < c_num_schunks * c_num_schunks; schunk++) {
		...
		char fname[128];    // Set up name.
		auto ireg_ds = GameDat::get()->Open_ODataSource(get_schunk_file_name(U7IREG, schunk, fname));

		if (!schunk_cache[schunk].empty()) {
			// It's cached to a memory buffer
			if (ireg_ds) {
				ireg_ds.write(schunk_cache[schunk].data(), schunk_cache[schunk].size());
			}
...
void Game_map::get_ireg_objects(int schunk    // Superchunk # (0-143).
	...
	if (!schunk_cache[schunk].empty()) {
		ireg = std::make_unique<IBufferDataView>(schunk_cache[schunk].data(), schunk_cache[schunk].size());
#ifdef DEBUG
