...
public:
	...
	bool operator==(const Tile_coord& t2) const {
	...
	int distance(const Tile_coord& t2) const {    // Distance to another tile?
	...
	int distance_2d(const Tile_coord& t2) const {    // For pathfinder.
	...
	int square_distance_2d(const Tile_coord& t2) const {
		int dy = (t2.ty - ty + c_num_tiles) % c_num_tiles;
		int dx = (t2.tx - tx + c_num_tiles) % c_num_tiles;
		if (dy >= c_num_tiles / 2) {
			dy = c_num_tiles - dy;
	...
	int square_distance_3d(const Tile_coord& t2) const {
		int       dy = (t2.ty - ty + c_num_tiles) % c_num_tiles;
		int       dx = (t2.tx - tx + c_num_tiles) % c_num_tiles;
	...
	int square_distance_screen_space(const Tile_coord& t2) const {
		int       dy = (t2.ty - ty + c_num_tiles) % c_num_tiles;
		int       dx = (t2.tx - tx + c_num_tiles) % c_num_tiles;
		const int dz = t2.tz - tz;
		if (dy >= c_num_tiles / 2) {
	...
	static bool gte(int t1, int t2) {    // Ret. t1 >= t2 with wrapping.
		...
		return diff >= 0 ? (diff < c_num_tiles / 2) : (diff < -c_num_tiles / 2);
	}
...
// Add two coords.
inline Tile_coord operator+(const Tile_coord& a, const Tile_coord& b) {
