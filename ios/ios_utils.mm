...
- (void)setupQuickActions {
	NSString* documentsPath = IOSGetDocumentsDir() ? [NSString stringWithUTF8String:IOSGetDocumentsDir()] : nil;
	if (!documentsPath) {
		return;
	}
	...
	if ([self isGameInstalled:@"bg"]) {
		UIApplicationShortcutItem* bgShortcut = [[UIApplicationShortcutItem alloc]
	...
	if ([self isGameInstalled:@"si"]) {
		UIApplicationShortcutItem* siShortcut = [[UIApplicationShortcutItem alloc]
	...
	if ([self isGameInstalled:@"fov"]) {
		UIApplicationShortcutItem* fovShortcut = [[UIApplicationShortcutItem alloc]
	...
	if ([self isGameInstalled:@"ss"]) {
		UIApplicationShortcutItem* ssShortcut = [[UIApplicationShortcutItem alloc]
	...
	if ([self isGameInstalled:@"sib"]) {
		UIApplicationShortcutItem* sibShortcut = [[UIApplicationShortcutItem alloc]
...
- (BOOL)isGameInstalled:(NSString*)gameType {
	...
	BOOL (^checkStaticFile)(NSString*) = ^BOOL(NSString* basePath) {
	  NSString* staticLower = [basePath stringByAppendingPathComponent:@"static"];
	  NSString* staticUpper = [basePath stringByAppendingPathComponent:@"STATIC"];

	  BOOL lowerExists = [[NSFileManager defaultManager] fileExistsAtPath:staticLower];
	  BOOL upperExists = [[NSFileManager defaultManager] fileExistsAtPath:staticUpper];

	  return lowerExists || upperExists;
	};

	if ([gameType isEqualToString:@"bg"]) {
		NSString* bgPath = [gamePath stringByAppendingPathComponent:@"blackgate"];
		return checkStaticFile(bgPath);
	} else if ([gameType isEqualToString:@"si"]) {
		NSString* siPath = [gamePath stringByAppendingPathComponent:@"serpentisle"];
		return checkStaticFile(siPath);
	} else if ([gameType isEqualToString:@"fov"]) {
		NSString* bgPath  = [gamePath stringByAppendingPathComponent:@"blackgate"];
		NSString* fovPath = [gamePath stringByAppendingPathComponent:@"forgeofvirtue"];
		return checkStaticFile(bgPath) && checkStaticFile(fovPath);
	} else if ([gameType isEqualToString:@"ss"]) {
		NSString* siPath = [gamePath stringByAppendingPathComponent:@"serpentisle"];
		NSString* ssPath = [gamePath stringByAppendingPathComponent:@"silverseed"];
		return checkStaticFile(siPath) && checkStaticFile(ssPath);
	} else if ([gameType isEqualToString:@"sib"]) {
		NSString* sibPath = [gamePath stringByAppendingPathComponent:@"serpentbeta"];
...
- (void)sendRecurringKeycode {
	...
	event.key.key      = SDL_GetKeyFromScancode(self.recurringKeycode, SDL_KMOD_NONE, false);
	event.key.down     = true;
	event.key.repeat   = false;
	event.key.windowID = 0;    // keyboard->focus ? keyboard->focus->id : 0;
	SDL_PushEvent(&event);
...
- (void)buttonUp:(GamePadButton*)btn {
	...
	if (btn == self.btn2) {
		...
		btn.title     = [cur isEqualToString:kGlyphPause] ? kGlyphPlay : kGlyphPause;
	}
	//	SDL_SendKeyboardKey(0, 0, 0, keycode, SDL_RELEASED);
	SDL_Event event;
	SDL_zero(event);
...
- (CGRect)calcRectForDPad {
	...
	CGRect rcDpad = CGRectMake(left, rcScreen.size.height - sizeDpad.height - margin, sizeDpad.width, sizeDpad.height);
	return rcDpad;
}
...
- (void)layoutButtonsForDpadLocation {
	...
	CGRect rcEsc;
	if (escOnLeft) {
		rcEsc = CGRectMake(btnMargin, rcScreen.size.height - sizeButton.height - btnMargin, sizeButton.width, sizeButton.height);
	} else {
		rcEsc = CGRectMake(
				rcScreen.size.width - sizeButton.width - btnMargin, rcScreen.size.height - sizeButton.height - btnMargin,
	...
	if (self.btn2 && self.btn2.alpha > 0.0) {
		CGRect rcPause;
		if (escOnLeft) {
			rcPause = CGRectMake(CGRectGetMaxX(rcEsc) + gap, rcEsc.origin.y, sizeButton.width, sizeButton.height);
		} else {
			rcPause = CGRectMake(
					CGRectGetMinX(rcEsc) - gap - sizeButton.width, rcEsc.origin.y, sizeButton.width, sizeButton.height);
		}
...
- (void)showButtonControls {
	if (self.btn1 == nil) {
		self.btn1 = [self createButton:@"ESC" keycode:static_cast<int>(SDL_SCANCODE_ESCAPE) rect:CGRectZero];
	}
...
- (void)showPauseControls {
	...
	UIViewController* controller = window.rootViewController;
	if (self.btn2 == nil) {
		self.btn2 = [self createButton:kGlyphPause keycode:static_cast<int>(SDL_SCANCODE_SPACE) rect:CGRectZero];
	}
...
+ (void)load {
	...
	[[NSNotificationCenter defaultCenter] addObserver:[ExultQuickActionManager sharedManager]
