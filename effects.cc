...
BufferTransitionEffect::BufferTransitionEffect(uint64 duration, TileRect effectRect) : startTime(0), duration(duration) {
...
void BufferTransitionEffect::UpdateRect(const TileRect& rect) {
	...
	if (int(buffer1->get_width()) != rect.w || int(buffer1->get_height()) != rect.h) {
		auto gwin = Game_window::get_instance();
		auto win  = gwin->get_win();
...
void PageTurnEffect::paint() {
	...
	float angle   = std::min(1.0f, static_cast<float>(getElapsedTime()) / duration) * M_PI;
	...
	for (float x = 0;; x += xinc) {
		...
		if (!alternativeEffect) {
			...
			ibuf->copy_col8(src, ox + srcx, oy, pageHeight, ox + pageWidth + x, oy + y);
		} else {
			// alternative effect without scaling page content
			ibuf->copy_col8(src, ox + pageWidth + x, oy, pageHeight, ox + pageWidth + x, oy + y);
		}
	}

	if (lineColour != 0xff) {
		// Top line
		ibuf->draw_line8(lineColour, ox + pageWidth, oy, ox + pageWidth + cornerX, oy + cornerY);
		// bottom line
		ibuf->draw_line8(lineColour, ox + pageWidth, oy + pageHeight, ox + pageWidth + cornerX, oy + cornerY + pageHeight);
		// edge line
		ibuf->draw_line8(lineColour, ox + pageWidth + cornerX, oy + cornerY, ox + pageWidth + cornerX, oy + cornerY + pageHeight);
	}
}
