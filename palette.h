...
public:
	...
	void                     create_palette_map(const Palette* to, unsigned char* buf, bool keep_rotating = false) const;
	std::unique_ptr<Palette> create_intermediate(const Palette& to, int nsteps, int pos, bool apply = true) const;
	// Create a intermediate palette between pal1 and pal2 of this Palette
	std::unique_ptr<Palette> create_fadeintermediate(int nsteps, int pos) const;
