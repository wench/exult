...
Game_window::Game_window(
		...
		: dragging(nullptr), effects(new Effects_manager(this)), map(new Game_map(0)), render(new Game_render),
		  ...
		  scrollty(0), dirty(0, 0, 0, 0), mouse3rd(false), fastmouse(false), double_click_closes_gumps(false), text_bg(false),
		  step_tile_delta(8), allow_right_pathfind(2), scroll_with_mouse(false), alternate_drop(false), allow_autonotes(false),
		  allow_enhancements(false), in_exult_menu(false), extended_intro(false), load_palette_timer(0), plasma_start_color(0),
		  plasma_cycle_range(0), skip_lift(255), paint_eggs(false), paint_egg_areas(0), armageddon(false), walk_in_formation(false),
		  debug(0), blits(0), scrolltx_l(0), scrollty_l(0), scrolltx_lp(0), scrollty_lp(0), scrolltx_lo(0), scrollty_lo(0),
...
void Game_window::setup_game(bool map_editing) {
	...
	// load saved palette or set palette for time-of-day.
	...
	// example autosaves during moon gate travel will have a faded out palette and fading in here will cause a double fade in when
