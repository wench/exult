...
namespace Cache {
	...
	static U7multiobject& Load(const File_spec& fname0, const File_spec& fname1, const File_spec& fname2, int index) {
		// Key uses real paths
		KeyType key = {
		...
		if (it != loaded.end()) {
			...
			// 3 Filenames
			auto res = loaded.emplace(key, U7multiobject(std::get<0>(key), std::get<1>(key), std::get<2>(key), std::get<3>(key)));
			if (res.second) {
			...
			// 2 Filenames
			auto res = loaded.emplace(key, U7multiobject(std::get<0>(key), std::get<1>(key), std::get<3>(key)));
			...
			// 1 filename
			auto res = loaded.emplace(key, U7multiobject(std::get<0>(key), std::get<3>(key)));
...
void Palette::create_palette_map(const Palette* to, unsigned char* buf, bool keep_rotating) const {
	// Assume buf has 256 elements
	int limit = keep_rotating ? 0xe0 : 256;
	for (int i = 0; i < limit; i++) {
		buf[i] = to->find_color(pal1[3 * i], pal1[3 * i + 1], pal1[3 * i + 2], limit);
	}
...
std::unique_ptr<Palette> Palette::create_intermediate(const Palette& to, int nsteps, int pos, bool apply) const {
...
std::unique_ptr<Palette> Palette::create_fadeintermediate(int nsteps, int pos) const {
	auto newpal = std::make_unique<Palette>();
	if (fades_enabled) {
		for (int c = 0; c < 768; c++) {
