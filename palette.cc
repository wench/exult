...
namespace Cache
{
	...
	static U7multiobject& Load(
		const File_spec& fname0, const File_spec& fname1, const File_spec& fname2,
		int index)
	{
		// Key uses real paths
		KeyType key
				= {std::string(get_system_path(fname0.name)), std::string(get_system_path(fname1.name)),
				   std::string(get_system_path(fname2.name)), index};
		...
		if (it != loaded.end())
		{
			...
			// 3 Filenames
			auto res = loaded.emplace(key, U7multiobject(std::get<0>(key), std::get<1>(key),
					std::get<2>(key),
					std::get<3>(key)));
			if (res.second) {
			...
			auto res = loaded.emplace(
					key,
					U7multiobject(std::get<0>(key), std::get<1>(key),std::get<3>(key)));
